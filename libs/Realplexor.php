...
class Dklab_Realplexor
{
	 ...
	 * @return void
	 */                              
	public function send($idsAndCursors, $data, $showOnlyForIds = null, $isDebug = false, $ignoreResponce = false)
	{
if ($isDebug == true)
{
	$_di = 0;
	$_debug[$_di] = Array('ts' => microtime(true), 'name' => 'dklab_send_start', 'data' => null);
}		
...
if ($isDebug == true)
{
	$_debug[] = Array('ts' => (microtime(true) - $_debug[0]['ts']), 'name' => 'Prepare params', 'data' => null);
}		
		
		$_sendDebug = $this->_send(join(",", $pairs), $data, $isDebug, $ignoreResponce);
		
if ($isDebug == true)
{
	$_debug[] = Array('ts' => (microtime(true) - $_debug[0]['ts']), 'name' => 'Low-level send', 'data' => $_sendDebug);
	 ...
	 */
	private function _send($identifier, $body, $isDebug = false, $ignoreResponce = false)
	{
if ($isDebug == true)
{
	$_di = 0;
...
if ($isDebug == true)
{
	$_debug[] = Array('ts' => (microtime(true) - $_debug[0]['ts']), 'name' => 'Prepare HTTP', 'data' => null);
}		
		...
		try {
			...
			if ($ignoreResponce == true)
			{
				if (!@stream_socket_shutdown($f, STREAM_SHUT_RDWR))
					throw new Dklab_Realplexor_Exception($php_errormsg);
				return true;
			}
...
if ($isDebug == true)
{
	$_debug[] = Array('ts' => (microtime(true) - $_debug[0]['ts']), 'name' => 'Connect-send via socket', 'data' => null);
}			
...
if ($isDebug == true)
{
	$_debug[] = Array('ts' => (microtime(true) - $_debug[0]['ts']), 'name' => 'Fetching results', 'data' => $result);
}	
...
if ($isDebug == true)
{
	$_debug[] = Array('ts' => (microtime(true) - $_debug[0]['ts']), 'name' => 'Processing body of results', 'data' => null);
	return $_debug;
}
